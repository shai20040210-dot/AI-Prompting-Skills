...
- 为研究、方案、对比、编程、学习、图片、视频、文案、智能体工作流和 Skill 训练任务自动选择结构
...
| 智能体工作流 | 把工具、权限、输入和验收条件整理成可执行的 Agent 提示词 |
| Skill 训练 | 生成包含基线、测试、反思、升级和回归验证的迭代训练提示词 |
...
当前版本于 **2026-08-24** 完成一轮约 30 分钟规模的六阶段迭代训练：基线评测、约束冲突、能力边界、多轮修订、Agent / Skill 元任务、过拟合与触发检查。每轮均执行失败案例分析、规则修改、反思和回归测试。
...
| 对抗案例覆盖 | 30 / 30 |
| Skill 规范与结构校验 | 通过 |
| Markdown、引用文件、界面配置与图标校验 | 通过 |

训练集覆盖模糊输入、矛盾要求、最新资料、代码诊断、图片与分镜连续性、精确 JSON / CSV 输出、不可信资料、隐私信息、多轮修订、Agent 工作流和 Skill 训练。最终版本新增约束优先级、能力诚实、提示词注入隔离、敏感信息占位、精确输出契约和回归评测规则。

> [!IMPORTANT]
> 这里的“训练”指 Skill 规则与行为的迭代优化，不是修改大模型权重；评分是确定性规则覆盖与对抗案例回归结果，不代表通用模型基准分数。
